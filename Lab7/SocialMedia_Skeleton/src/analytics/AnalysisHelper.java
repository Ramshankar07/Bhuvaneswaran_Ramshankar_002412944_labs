...
public class AnalysisHelper {
    ...
    public void getPostWithMostComments() {
        ...
        for (Post p : data.getPosts().values()) {
            if (postWithMostComments == null || p.getComments().size() > postWithMostComments.getComments().size()) {
    ...
    public void getPassiveUser() {
     ...
     public void getPassiveCommetUsers() {
        DataStore data = DataStore.getInstance();
 ...
 public void getPassiveAndActiveOverallUsers() {
        ...
        Collections.sort(users, new UserMapComparator(overallNumbers));
        System.out.println("Q7- The following user have overall been active:");
